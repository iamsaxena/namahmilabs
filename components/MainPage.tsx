    ...
    <main id="top" className="relative overflow-hidden bg-background">
          ...
          <div className="grid auto-rows-fr gap-5 md:grid-cols-3">
            {ventures.map((venture, index) => (
              ...
              >
                ...
                <h3 className="font-heading text-3xl font-semibold">{venture.name}</h3>
      ...
      <section className="relative z-10 px-5 py-28 md:px-8">
        <div className="mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-[0.85fr_1.15fr]">
          ...
          >
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_25%,rgba(6,182,212,0.35),transparent_34%),linear-gradient(135deg,rgba(37,99,235,0.25),rgba(139,92,246,0.14))]" />
            <div className="absolute inset-8 flex items-center justify-center rounded-full border border-white/10">
              <div className="flex h-40 w-40 items-center justify-center rounded-full bg-white text-background shadow-glow">
                <span className="font-heading text-5xl font-semibold">SS</span>
              </div>
            </div>
          </motion.div>
          <div>
            <SectionLabel>Founder</SectionLabel>
            <motion.h2 variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="font-heading text-5xl font-semibold md:text-7xl">
              Shobhit Shubham Saxena
            </motion.h2>
            <motion.p variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} transition={{ delay: 0.08 }} className="mt-8 max-w-3xl text-2xl leading-10 text-white/90">
              “Innovation begins with purpose. Purpose becomes impact through research.”
            </motion.p>
            <motion.p variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} transition={{ delay: 0.16 }} className="mt-8 max-w-3xl leading-8 text-muted">
              Namahmi Labs is a place where researchers, engineers, educators, healthcare experts, creators and entrepreneurs can come together to solve challenges that matter.
            </motion.p>
          </div>
        </div>
      </section>
